...
Demo integration:
- site/demo/flagship-demo.html — the playable demo page shell: on-screen keys, waveform/ADSR/filter-cutoff controls, a preset picker, and a single "<script src=\"flagship-demo.js\">" tag.
- site/demo/flagship-demo.js — the Web Audio logic: creates one AudioContext, wires noteOn/noteOff through a shared createVoice() helper (oscillator -> biquad lowpass filter -> gain), builds the on-screen keys, maps the computer keyboard row A S D F G H J K to one octave, and loads "../presets/flagship-presets.json" to populate and apply presets (waveform and filter cutoff).

flagship.html does not hard-code an iframe to the demo: the section "<div id=\"demo-slot\" data-demo-src=\"../demo/flagship-demo.html\">" shows a plain "not published yet" message, and an inline script fetches that path and swaps in an iframe (title "Flagship demo", 360px tall) only when the response is ok — now that site/demo/flagship-demo.html exists, the page shows the working demo instead of the fallback message. Presets are loaded independently by both flagship.html and the demo from "../presets/flagship-presets.json".
