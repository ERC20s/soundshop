...
Demo integration:
site/demo/flagship-demo.html is the playable in-browser flagship synth demo: a dependency-free page wiring an oscillator (sine/saw/triangle/square, selectable) into a resonant BiquadFilterNode low-pass into a gain node driven by ADSR (attack/decay/sustain/release) sliders, playable from on-screen keys or the computer keyboard (A W S E D F T G Y H U J K). It also loads site/presets/flagship-presets.json and lets you apply a preset's osc1 waveform and filter cutoff. flagship.html does not hard-code an iframe to it: the section "<div id=\"demo-slot\" data-demo-src=\"../demo/flagship-demo.html\">" shows a plain "not published yet" message until an inline script fetches that path and swaps in an iframe (title "Flagship demo", 360px tall) once the response is ok — which it now is. Presets are loaded independently by both pages from "../presets/flagship-presets.json".
