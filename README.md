...
- site/demo/flagship-demo.html — the playable in-browser flagship synth demo: an on-screen keyboard (also playable from mapped computer keys A W S E D F T G Y H U J K), a waveform selector (sine/saw/triangle/square), a resonant low-pass filter (cutoff + resonance) and ADSR sliders on the amp envelope. An OscillatorNode feeds a BiquadFilterNode into a GainNode whose gain follows the ADSR envelope; the AudioContext is created/resumed on the first key press or click, per browser autoplay rules. It also offers an optional "Load preset" dropdown that reads site/presets/flagship-presets.json and applies a preset's waveform and filter cutoff.
...
Demo integration:
site/demo/flagship-demo.html now exists, so the flagship page's probe embeds it automatically. flagship.html does not hard-code an iframe: the section "<div id=\"demo-slot\" data-demo-src=\"../demo/flagship-demo.html\">" shows a plain "not published yet" message until an inline script fetches that path and swaps in an iframe (title "Flagship demo", 360px tall) once the response is ok — no change to flagship.html was needed. Presets are still loaded from "../presets/flagship-presets.json", both by the plugin page's short list and by the demo's own preset dropdown.
