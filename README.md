...
- site/index.html — home page; the playable keyboard is the first block in <main> (it loads "plugins/demo-keys.js" into its own "<div id=\"demo-slot\">"), followed by the links to plugin pages, the presets gallery and changelog.
...
Playable demo:
The demo lives on the pages themselves — there is no site/demo/ directory and no iframe. It appears twice: on site/index.html, where it is the first section in <main> and loads "plugins/demo-keys.js", and on site/plugins/flagship.html, where the "<section id=\"demo\">" now sits above "<section id=\"specs\">" and loads "demo-keys.js" from the same directory. Both use a deferred script tag; that file builds a one-octave keyboard (C4-C5), a waveform select and a 200-8000 Hz cutoff slider inside "<div id=\"demo-slot\">", but only when the browser exposes AudioContext/webkitAudioContext. Otherwise the fallback paragraph in the slot stays untouched. Notes play on pointerdown and on the computer keys a w s e d f t g y h u j k, release on pointerup/keyup, and are capped at 8 simultaneous voices (oldest stolen first) to match the "Polyphony: 8 voices" spec. The voice graph — oscillator to lowpass biquad to gain — is the same one used by site/presets/preview.js, and the AudioContext is created lazily on the first interaction so autoplay policies are respected. Styles live under .demo-keys / .demo-key / .demo-controls in site/assets/style.css. Presets are still loaded from "../presets/flagship-presets.json".
