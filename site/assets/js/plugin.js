...
(function (window, document) {
  ...
  /**
   * Extract a conservative receipt URL from an order-like object. This is
   * intentionally narrow: only explicit http(s) values are accepted.
   */
        ...
        try {
          ...
          if (parsed && typeof parsed === 'object') {
            ...
            for (var tok in parsed) {
              ...
              // Preserve any receipt URL stored in the record
              try {
    ...
    try {
      ...
      try {
        ...
        if (already === 'on') {
          ...
          } else if (detail && !extractDownloadUrl(detail)) {
            // No download URL but perhaps a provider receipt URL is available.
            var r = detail.receiptUrl || extractReceiptUrl(detail);
            if (r) {
              try {
                var existing = host.querySelector('.bought__cta');
                if (existing && existing.tagName && existing.tagName.toLowerCase() === 'a') {
                  try { existing.setAttribute('href', r); } catch (e) { /* ignore */ }
                  try { existing.setAttribute('target', '_blank'); } catch (e) { /* ignore */ }
                  try { existing.setAttribute('rel', 'noopener noreferrer'); } catch (e) { /* ignore */ }
      ...
      try {
        ...
        } else if (detail && (detail.receiptUrl || extractReceiptUrl(detail))) {
          // No installer available, but we can surface a provider receipt as a
          // safe fallback so users can find their payment receipt from here.
          try { makeReceiptAnchor(detail.receiptUrl || extractReceiptUrl(detail)); } catch (e) { /* ignore */ }
        } else {
                ...
                p.then(function (order) {
                  try {
                    ...
                    if (v.downloadUrl) {
                      ...
                      // We have no installer but the platform returned a receipt
                      // URL. Store it in the detail and persist to canonical storage
                      // so the bought-note/summary can show a safe "View receipt"
                      // fallback.
          ...
          p.then(function (order) {
            try {
              ...
              if (v.downloadUrl) {
                ...
                // No installer URL, but a provider receipt URL is present. Save
                // it into the remembered item and persist canonical storage so
                // other pages can surface the safe fallback.
