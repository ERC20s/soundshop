...
(function (window, document) {
  ...
  P.initBoughtSummary = function (root) {
    $$('[data-bought-summary]', root || document).forEach(function (host) {
      ...
      try {
        var raw = attr(host, 'data-bought-items');
      ...
      items.forEach(function (it) {
        try {
          ...
          var labelSpan = el('span', 'bought__label', it.label || it.name || it.itemName || 'Purchased item');
          labelSpan.style.fontWeight = '600';

          var metaSpan = el('span', 'bought__meta');

          var ref = it.paymentReference || it.reference || it.ref || '';
          var email = it.deliveryEmail || it.delivery || it.email || '';
          var norefMsg = it.message || '';
  ...
  try {
    document.addEventListener('soundshop:verified-order', function (ev) {
      try {
        ...
        // Open installers/delivery in a new tab and provide an accessible label
        ...
        // Keep this as a normal link; do not set role="button"
        ...
        // Attempt to focus the primary CTA, but swallow failures (eg. display:none)
