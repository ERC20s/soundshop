  ...
  try {
    document.addEventListener('soundshop:verified-order', function (ev) {
      try {
        ...
        // Make clear this link opens a new tab/window for users and do so safely
        try { a1.setAttribute('target', '_blank'); } catch (e) { /* ignore */ }
        try { a1.setAttribute('rel', 'noopener noreferrer'); } catch (e) { /* ignore */ }
        ...
        // Secondary link also opens in a new tab for consistency and safety
        try { a2.setAttribute('target', '_blank'); } catch (e) { /* ignore */ }
        try { a2.setAttribute('rel', 'noopener noreferrer'); } catch (e) { /* ignore */ }

        // Add a short, polite aria-live status so screen readers get an explicit announcement
        var statusSpan = document.createElement('span');
        statusSpan.setAttribute('aria-live', 'polite');
        // Visually hide without relying on external CSS rules
        try {
          statusSpan.style.position = 'absolute';
          statusSpan.style.left = '-9999px';
          statusSpan.style.width = '1px';
          statusSpan.style.height = '1px';
          statusSpan.style.overflow = 'hidden';
          statusSpan.style.clip = 'rect(0 0 0 0)';
          statusSpan.style.whiteSpace = 'nowrap';
        } catch (e) { /* ignore styling errors */ }
        try { statusSpan.textContent = 'Installers and delivery instructions available — link focused'; } catch (e) { /* ignore */ }

        c.appendChild(a1);
        c.appendChild(a2);
        c.appendChild(statusSpan);
        ...
        // Best-effort move keyboard focus to the primary CTA so keyboard/screen-reader users land on it
        try { a1.focus(); } catch (e) { /* ignore focus errors */ }
