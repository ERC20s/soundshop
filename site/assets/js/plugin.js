...
(function (window, document) {
  ...
  // Add a small, user-initiated URL-order verify UI for the case where the
  // payments widget is absent but the user has returned with ?d8a_order=<id>.
  // This avoids silent network probes and gives the buyer a clear affordance
  // to confirm their purchase and recover the Download CTA.
  P.initUrlOrderVerifyUi = function () {
    try {
      // Only run once per document
      if (document.body && document.body.getAttribute && document.body.getAttribute('data-ssp-url-order-verify') === 'on') return;
      if (document.body && document.body.setAttribute) document.body.setAttribute('data-ssp-url-order-verify', 'on');

      var m = (location.search && location.search.match(/[?&]d8a_order=([A-Za-z0-9_-]+)/)) || (location.href && location.href.match(/[?&]d8a_order=([A-Za-z0-9_-]+)/));
      var id = m && m[1] ? String(m[1]) : '';
      if (!id) return;

      // If the payments widget is present it already exposes verification UI.
      if (typeof window.groupStoreVerify === 'function') return;

      // Build a small banner
      var banner = document.createElement('div');
      banner.className = 'ssp-url-order-verify';
      banner.style.cssText = 'background:#f8fafc;border:1px solid #e5e7eb;padding:10px 12px;margin:0 0 12px 0;font:13px system-ui,sans-serif;color:#111;display:flex;align-items:center;gap:12px;justify-content:space-between';

      var left = document.createElement('div');
      left.style.cssText = 'flex:1';
      left.textContent = 'You returned from checkout on this device — click Verify to confirm your purchase.';

      var actions = document.createElement('div');
      actions.style.cssText = 'flex:0 0 auto;display:flex;align-items:center;gap:8px';

      var btn = document.createElement('button');
      btn.className = 'btn btn-ghost';
      try { btn.setAttribute('type', 'button'); } catch (e) { /* ignore */ }
      btn.textContent = 'Verify purchase';
      btn.setAttribute('data-ssp-url-order-id', id);
      ...
      // Insert banner before main content when available, otherwise at top of body
      var insertBefore = $('main') || document.body.firstElementChild || null;
      try {
        if (insertBefore && insertBefore.parentNode) insertBefore.parentNode.insertBefore(banner, insertBefore);
        else document.body.insertBefore(banner, document.body.firstChild);
      } catch (e) { /* ignore insertion */ }

      // One-shot click handler for the button
      btn.addEventListener('click', function () {
        try {
          ...
          try { btn.textContent = 'Verifying…'; } catch (e) { /* ignore */ }

          var url = 'https://d8a.com/api/v1/store/orders/' + encodeURIComponent(id) + '?group=batch-synthshop';

          fetch(url, { method: 'GET', credentials: 'omit' }).then(function (r) {
            if (!r || !r.ok) throw new Error('network');
            try { return r.json(); } catch (e) { throw new Error('parse'); }
          }).then(function (json) {
            try {
              // Normalise to an order object when possible. The platform
              // usually returns { paid: true, order: { ... } } but older or
              // alternative responses may be the order itself.
              var order = null;
              if (json && typeof json === 'object') {
                if (json.paid && json.order) order = json.order;
                else if (json.paid && (json.id || json.orderId)) order = json.order || json;
                else if (json.id) order = json; // assume it's an order
              }

              if (!order) {
                // show brief support hint
                try {
                  left.textContent = 'Could not verify this order here. Need help?';
                  var a = document.createElement('a');
                  a.setAttribute('href', 'docs.html#support');
                  a.style.color = '#7c5cff';
                  a.textContent = ' Contact Support';
                  // append as a separate node
                  if (left && left.appendChild) left.appendChild(a);
                } catch (e) { /* ignore */ }
                throw new Error('no-order');
              }

              // Persist discovered order conservatively
              try { if (typeof window.soundshopPersistBought === 'function') window.soundshopPersistBought(order); } catch (e) { /* ignore */ }

              // Dispatch an in-page event for other code to react
              try { document.dispatchEvent(new CustomEvent('soundshop:verified-order', { detail: order })); } catch (e) { /* ignore */ }

              // Update banner to success and remove after a short delay
              try {
                banner.style.borderColor = '#10b981';
                left.textContent = 'Verified — your purchase has been confirmed on this device.';
                // Replace actions with a dismiss button
                actions.innerHTML = '';
                var dismiss = document.createElement('button');
                dismiss.className = 'btn';
                try { dismiss.setAttribute('type', 'button'); } catch (e) { /* ignore */ }
                dismiss.textContent = 'Close';
                actions.appendChild(dismiss);
                dismiss.addEventListener('click', function () { try { if (banner && banner.parentNode) banner.parentNode.removeChild(banner); } catch (e) { /* ignore */ } });
                // Auto-remove after 6s to reduce persistent UI
                setTimeout(function () { try { if (banner && banner.parentNode) banner.parentNode.removeChild(banner); } catch (e) { /* ignore */ } }, 6000);
              } catch (e) { /* ignore */ }
            ...
            // Any failure shows an inline support hint and restores the button
            try {
              ...
              a.setAttribute('href', 'docs.html#support');
              a.style.color = '#7c5cff';
              a.textContent = 'Need help? Contact Support';
              hint.appendChild(a);
              if (actions && actions.appendChild) actions.appendChild(hint);
            } catch (e) { /* ignore */ }
            try { btn.textContent = orig; } catch (e) { /* ignore */ }
  ...
  P.init = function () {
    ...
    // Initialise URL order verify UI (user-initiated) so buyers who return
    // with ?d8a_order=<id> but without the payments widget can verify it.
