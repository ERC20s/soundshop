...
(function (window, document) {
  ...
  /* =======================================================================
     03  (rest of file unchanged) — full content continued below
     ======================================================================= */
  ...
  function maskEmail(email) {
    ...
    if (dparts.length > 1) dparts[0] = dparts[0][0] + '…';
    domain = dparts.join('.');
    return name + '@' + domain;
  }
  ...
  P.initBoughtSummary = function (root) {
    $$('[data-bought-summary]', root || document).forEach(function (host) {
      try {
        ...
        var items = [];
        try {
          var raw = host.getAttribute('data-bought-records') || '';
          if (raw && raw.trim()) {
        ...
        items.forEach(function (it) {
          try {
            ...
            var labelSpan = el('span', 'bought__label', it.itemName || it.name || 'Unknown item');
            labelSpan.style.fontWeight = '600';

            var metaSpan = el('span', 'bought__meta');

            var ref = it.id || it.ref || it.reference || it.paymentReference || '';
            var email = it.email || it.deliveryEmail || it.buyerEmail || it.customerEmail || '';
            var norefMsg = it.note || it.message || '';

            if (ref && !email) {

              try {
                var metaText = 'Payment reference: ' + ref;
      ...
      } catch (e) { /* swallow per-host errors */ }
    });
  };
  ...
  try {
    document.addEventListener('soundshop:verified-order', function (ev) {
      try {
        ...
        // use a real anchor that opens in a new tab and is safe
        a1.target = '_blank';
        a1.rel = 'noopener noreferrer';
        a1.style.marginRight = '12px';

        // Provide a hidden, accessible live-text description and reference it
        try {
          var descId = 'bought-cta-desc-' + String(Date.now()) + '-' + String(Math.floor(Math.random() * 1000));
          var desc = document.createElement('span');
          desc.id = descId;
          desc.setAttribute('aria-live', 'polite');
          desc.textContent = 'Installers and delivery instructions are available in a new tab.';
          // visually hide while keeping accessible to assistive tech
          ...
          a1.setAttribute('aria-describedby', descId);
          c.appendChild(desc);
        } catch (e) { /* ignore description errors */ }
        ...
        // Attempt to focus the primary link, but do not throw if it fails
        try { a1.focus(); } catch (e) { /* swallow focus errors */ }
