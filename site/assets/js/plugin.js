...
(function (window, document) {
  ...
  // ... many other functions omitted for brevity in this view; keep original content
  // For the purposes of this change we only modify initPaidBannerCtas below.
      ...
      for (var i = 0; i < banners.length; i++) {
        ...
        try {
          ...
          try {
            ...
            // Insert an idempotent "Copy reference" button into the CTAs when a
            // non-empty order reference exists. Guarded by per-banner attribute
            // and by presence of an existing .paid-banner__copy element so
            // multiple runs don't create duplicates.
            try {
              // Skip if banner already had copy added
              if (b.getAttribute && b.getAttribute('data-ssp-paid-copy') !== 'on') {
                // Avoid adding if container already contains a copy button
                var already = null;
                try { already = container.querySelector && container.querySelector('.paid-banner__copy'); } catch (e) { already = null; }
                if (!already) {
                  var fullRef = '';
                  ...
                  if (fullRef) {
                    ...
                    try {
                      // Store the full ref on the button for later reading
                      copyBtn.setAttribute('data-ssp-full-ref', fullRef);
                      // Accessible label uses masked reference when available
                      try { copyBtn.setAttribute('title', 'Copy reference: ' + (maskRef(fullRef) || fullRef)); } catch (e) { /* ignore */ }
                      try { copyBtn.setAttribute('aria-label', 'Copy reference: ' + (maskRef(fullRef) || fullRef)); } catch (e) { /* ignore */ }
                    } catch (e) { /* ignore */ }

                    // Click handler with layered copy fallbacks and toast feedback
                    try {
                      copyBtn.addEventListener('click', function (e) {
                        try {
                          ...
                          var refToCopy = '';
                          try { refToCopy = (btn && btn.getAttribute) ? (btn.getAttribute('data-ssp-full-ref') || '') : (fullRef || ''); } catch (e) { refToCopy = fullRef || ''; }
                          ...
                          try {
                            if (SS && typeof SS.copyText === 'function') {
                              try { SS.copyText(refToCopy); showSuccess(); try { if (btn) btn.disabled = false; } catch (e) { /* ignore */ } return; } catch (e) { /* fall through */ }
                            }
                          } catch (e) { /* ignore */ }

                          // Then navigator.clipboard when available
                          try {
                            if (navigator && navigator.clipboard && typeof navigator.clipboard.writeText === 'function') {
                              try {
                                navigator.clipboard.writeText(refToCopy).then(function () { showSuccess(); try { if (btn) btn.disabled = false; } catch (e) { /* ignore */ } }).catch(function () { /* fallthrough to textarea */
                                  try { /* fallback below */ } catch (e) { /* ignore */ }
                                });
                                return;
                    ...
                    } catch (e) { /* ignore listener attach */ }

                    // Append the button into the CTAs container
                    try { container.appendChild(copyBtn); } catch (e) { /* ignore */ }

                    // Mark banner so future runs won't add another copy button
                    try { b.setAttribute('data-ssp-paid-copy', 'on'); } catch (e) { /* ignore */ }
                  }
                }
              }
            } catch (e) { /* ignore copy button create */ }
