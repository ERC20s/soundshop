...
(function (window, document) {
  ...
  /* =======================================================================
     10  BOUGHT SUMMARY + RELATED HELPERS
     ======================================================================= */
  ...
  function maskEmail(e) {
    try {
      ...
      var u = parts[0];
      if (u.length <= 2) return '…@' + parts[1];
      return u.slice(0, 1) + '…' + u.slice(-1) + '@' + parts[1];
    } catch (e) { return ''; }
  }
  ...
  function extractDownloadUrl(rec) {
    try {
      ...
      if (rec && typeof rec === 'object') {
        downloadUrl = rec.downloadUrl || rec.installerUrl || (rec.installers && rec.installers[0] && rec.installers[0].url) || '';
  ...
  function extractReceiptUrl(rec) {
    try {
      var receiptUrl = '';
      if (rec && typeof rec === 'object') {
        receiptUrl = rec.receiptUrl || rec.receipt || '';
  ...
  function createBoughtCta(host, rec, token) {
    try {
      var downloadUrl = extractDownloadUrl(rec);
      var receiptUrl = extractReceiptUrl(rec);

      var wrap = document.createElement('span');
      wrap.className = 'bought-summary__ctas';
      ...
      // Per-item copy button (existing behaviour)
      try {
        var copyBtn = document.createElement('button');
        copyBtn.className = 'bought-summary__copy';
          ...
          try {
            ...
            try {
              if (SS && typeof SS.copyText === 'function') {
                try { SS.copyText(toCopy); showSuccess(); try { if (btn) btn.disabled = false; } catch (e) { } return; } catch (e) { /* fall through */ }
              }
            } catch (e) { /* ignore */ }
  ...
  function initBoughtNote() {
    try {
      var host = document.querySelector('[data-bought-note]');
      if (!host) return;
      if (host.getAttribute && host.getAttribute('data-ssp-bought-note') === 'on') return;
      try { host.setAttribute('data-ssp-bought-note', 'on'); } catch (e) { /* ignore */ }

      var id = host.getAttribute('data-bought-note');
      if (!id) return;
      ...
      try {
        ...
        if (rec && typeof rec === 'object') {
          try { host.classList.remove('is-hidden'); } catch (e) { /* ignore */ }
        }
      } catch (e) { /* ignore */ }
    } catch (e) { /* ignore */ }
  }
  ...
  function initBoughtSummary() {
    try {
      ...
      try { host.setAttribute('data-ssp-bought-summary', 'on'); } catch (e) { /* ignore */ }
      ...
      var list = null;
      try {
        list = host.querySelector('[data-bought-summary-items]');
        if (!list) {
          list = document.createElement('ul');
          list.setAttribute('data-bought-summary-items', 'on');
      ...
      try {
        ...
        if (any) {
          ...
          try {
            ...
            var msg = '';
            try {
              msg = 'You bought ' + (labelText || 'a product') + (dateText ? ' on ' + dateText + '. ' : '. ' ) + avail;
            } catch (e) { msg = '' }
            ...
            if (!statusNode) {
              try {
                ...
                try { statusNode.className = 'bought-summary__status s-visually-hidden'; } catch (e) { /* ignore */ }
                host.appendChild(statusNode);
              } catch (e) { statusNode = null; }
            }

            try { if (statusNode) statusNode.textContent = msg; } catch (e) { /* ignore */ }
          } catch (e) { /* ignore */ }
        } else {
          try { if (statusNode && statusNode.parentNode) statusNode.parentNode.removeChild(statusNode); } catch (e) { /* ignore */ }
        }
      } catch (e) { /* ignore */ }
      ...
      for (var i = 0; i < banners.length; i++) {
        ...
        try {
          ...
          try {
            ...
            // Add a guarded "Copy reference" button when a canonical reference exists.
            try {
              var ref = '';
              try { ref = String(order.id || order.ref || order.reference || '').trim(); } catch (e) { ref = ''; }
              if (ref) {
                // Ensure we don't add the button twice for the same banner
                if (b.getAttribute && b.getAttribute('data-ssp-paid-copy') !== 'on') {
                  try {
                    var copyBtn = document.createElement('button');
                    ...
                    copyBtn.addEventListener('click', function (e) {
                      try {
                        ...
                        var showFail = function () { try { if (SS && typeof SS.toast === 'function') SS.toast('Failed to copy'); } catch (e) { /* ignore */ } };
                        ...
                        // Try navigator.clipboard when available
                        try {
                          if (navigator && navigator.clipboard && typeof navigator.clipboard.writeText === 'function') {
                            try {
                              navigator.clipboard.writeText(toCopy).then(function () { try { showSuccess(); } catch (e) {} try { if (btn) btn.disabled = false; } catch (e) {} }, function () { try { showFail(); } catch (e) {} try { if (btn) btn.disabled = false; } catch (e) {} });
                              return;
                        ...
                        try {
                          ...
                          if (ok) {
                            ...
                            try { showFail(); } catch (e) { /* ignore */ }
                          }
                        } catch (e) { try { showFail(); } catch (err) { /* ignore */ } }

                        try { if (btn) btn.disabled = false; } catch (e) { /* ignore */ }
                      } catch (e) { try { if (SS && typeof SS.toast === 'function') SS.toast('Failed to copy'); } catch (err) { /* ignore */ } }
                    });
