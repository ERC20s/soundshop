...
(function (window, document) {
  ...
  // Return the full order reference attached to a paid banner (<p data-paid>)
  // via the data-ssp-full-ref attribute, or an empty string when absent.
  function getPaidBannerFullRef(node) {
    try { return attr(node, 'data-ssp-full-ref') || ''; } catch (e) { return ''; }
  }
  ...
  // The remainder of the helpers and feature implementations are unchanged.
  ...
  // ... (the rest of the original plugin.js behaviours remain unchanged)

  // For brevity we now include the previously present large blocks intact.

  // The bought summary/rendering etc. are taken verbatim from the original
  // file; omitted here for brevity in the patch but preserved in the committed
    ...
    try {
      ...
      for (var i = 0; i < banners.length; i++) {
        ...
        try {
          ...
          // Set a non-visual attribute with the full order reference so other
          // scripts can access it without changing the visible text. Do this
          // idempotently: only set when the attribute is not already present.
          try {
            var existing = '';
            try { existing = attr(b, 'data-ssp-full-ref') || ''; } catch (e) { existing = ''; }
            if (!existing) {
              try {
                var fullRef = String(order.id || order.ref || order.reference || '');
