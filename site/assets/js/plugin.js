...
(function (window, document) {
  ...
  P.initBoughtSummary = function (root) {
    $$('[data-bought-summary]', root || document).forEach(function (host) {
      ...
      try {
        var list = $('[data-bought-list]', host);
        var status = $('[data-bought-status]', host);

        // Load any records we already remember
        var boughtRecords = null;
        try { boughtRecords = JSON.parse(localStorage.getItem('boughtRecords') || '[]'); } catch (e) { boughtRecords = []; }

        var validCount = 0;
        boughtRecords.forEach(function (r) {
          try {
            ...
            emSpan.textContent = maskEmail(r.deliveryEmail || '');
  ...
  try {
    document.addEventListener('soundshop:verified-order', function (ev) {
      try {
        ...
        // Attempt to focus the primary CTA, but don't let focus failures break the page
