...
(function (window, document) {
  ...
  /**
   * Extract a conservative receipt URL from an order-like object.
   * This mirrors extractDownloadUrl but checks common receipt fields.
   */
    ...
    try {
      ...
      try {
        ...
        if (already === 'on') {
          ...
          // If we now have a receipt URL but no download URL, prefer rendering
          // a single external "View receipt" anchor as a conservative fallback.
      ...
      try {
        if (detail && extractDownloadUrl(detail)) {
          ...
          // If we have no download URL but do have a receipt URL, render the
          // conservative external "View receipt" anchor.
                ...
                p.then(function (order) {
                  try {
                    ...
                    if (v.downloadUrl) {
                      ...
                      // If verify returned only a receipt URL, persist it and
                      // update UI to show the conservative "View receipt" anchor.
                      ...
                      // Clear guards and re-run initialisers exactly like downloadUrl path
                      try {
                        $$('[data-bought-summary]').forEach(function (h) {
                          try {
                            var items = h.querySelectorAll('[data-ssp-paid-id]');
          ...
          p.then(function (order) {
            try {
              ...
              if (v.downloadUrl) {
                ...
                // If verify returned only a receipt URL, persist it and update UI
                ...
                // Clear guards and re-run initialisers so the new CTA is visible
                try {
                  $$('[data-bought-summary]').forEach(function (h) {
                    try {
                      var items = h.querySelectorAll('[data-ssp-paid-id]');
