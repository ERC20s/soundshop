...
(function (window, document) {
      ...
      try {
        ...
        if (!existing) {
          ...
          if (d) {
            ...
            try { setTimeout(function () { try { a.scrollIntoView({ behavior: reducedMotion() ? 'auto' : 'smooth', block: 'center' }); } catch (e) {} try { a.focus(); } catch (e) {} }, 0); } catch (e) {}
  ...
  function initBoughtSummary() {
    try {
      ...
      var list = el('div', 'bought-summary');
      list.setAttribute('data-bought-summary-list', '');

      try {
        var bought = readBoughtArray() || {};
        for (var k in bought) {
          if (!Object.prototype.hasOwnProperty.call(bought, k)) continue;
          var r = bought[k];
          try {
            var wrapper = renderBoughtRow(k, r);
            if (wrapper) list.appendChild(wrapper);
          } catch (e) { /* ignore row */ }
        }
      } catch (e) { /* ignore */ }
      ...
      try {
        document.addEventListener('group-store:paid', function (ev) {
          try {
            var o = ev && ev.detail ? ev.detail : null;
            if (o) {
              var token = getProductFromOrder(o);
              if (token) {
                var wrapper = renderBoughtRow(token, o);
                if (wrapper) {
                  try {
                    var existing = list.querySelector('[data-bought-row="' + token + '"]');
                    if (existing && existing.parentNode) existing.parentNode.replaceChild(wrapper, existing);
                    else list.appendChild(wrapper);
                  } catch (e) { /* ignore write */ }
                }
              }
            }
          } catch (e) { /* ignore */ }
  ...
  // initBoughtNote omitted for brevity in this file; it remains unchanged.
