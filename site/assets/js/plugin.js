...
(function (window, document) {
 ...
 // ... rest of file unchanged until updateUrlOrderBanner (kept for brevity)
      ...
      try {
        ...
        if (!existing) {
          ...
          if (d) {
            ...
            // Notify assistive tech by updating the banner text to include an
            // explicit mention that a download is available. Wrapped in try/catch
            // so failures in announcement do not affect the UI.
            try {
              if (textEl) {
                var base = textEl.textContent || '';
                if (base.indexOf('Download available') === -1) textEl.textContent = base + ' Download available.';
              }
            } catch (e) { /* ignore */ }
          } else {
  ...
  function initUrlOrderVerifyBanner() {
    try {
      ...
      // Announce updates to assistive technology in a non-interrupting way.
