        ...
        try {
          ...
          window.groupStoreVerify(orderId).then(function (o) {
            try {
              ...
              // Reveal and focus the Download CTA when present so returning buyers
              // can immediately find and activate their product. Defensive: guard
              // every operation and honour reduced motion.
              try {
                var cta = (banner && banner.querySelector) ? banner.querySelector('a.button.button--primary') : null;
                if (!cta) {
                  try { cta = document.querySelector('[data-bought-summary] a.button.button--primary'); } catch (e) { cta = null; }
                }
                if (cta) {
                  try { cta.scrollIntoView({ behavior: reducedMotion() ? 'auto' : 'smooth', block: 'center' }); } catch (e) { /* ignore */ }
