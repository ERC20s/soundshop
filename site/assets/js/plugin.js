...
(function (window, document) {
  ...
  // Remaining implementation omitted in this view for brevity; the file is long.
    ...
    if (any) {
      ...
      try {
        ...
        try {
          ...
          if (anyReceipt) avail = (avail ? avail + ' ' : '') + 'Receipt available.';
        } catch (e) { /* ignore */ }

        var msg = '';
        try {
          msg = 'You purchased ' + (labelText || 'items') + (dateText ? ' on ' + dateText + '.' : '.') + (avail ? ' ' + avail : '');
        } catch (e) { msg = 'You purchased items.'; }

        try {
          if (!statusNode) {
            statusNode = el('div', 'bought-summary__status');
            try { statusNode.setAttribute('data-bought-summary-status', 'on'); } catch (e) { /* ignore */ }
            try { statusNode.setAttribute('aria-live', 'polite'); } catch (e) { /* ignore */ }
            try { if (list && list.parentNode) list.parentNode.insertBefore(statusNode, list); else host.appendChild(statusNode); } catch (e) { try { host.appendChild(statusNode); } catch (err) { /* ignore */ } }
          }
          try { if (statusNode && typeof statusNode.textContent !== 'undefined') statusNode.textContent = msg; } catch (e) { /* ignore */ }
        } catch (e) { /* ignore */ }

      } catch (e) { /* ignore message build */ }
    }
  } catch (e) { /* ignore status update */ }
    ...
    try {
      ...
      for (var i = 0; i < banners.length; i++) {
        ...
        try {
          ...
          // Insert an idempotent "Copy reference" button when an order ref exists
          try {
            var fullRef = '';
            try { fullRef = String(order.id || order.ref || order.reference || '').trim(); } catch (e) { fullRef = ''; }
            if (fullRef) {
              try {
                // Skip if the banner already has this copy behaviour or a copy element
                if (b.getAttribute && b.getAttribute('data-ssp-paid-copy') === 'on') {
                  // already installed
                } else if (container && container.querySelector && container.querySelector('.paid-banner__copy')) {
                  // already present
                } else {
                  ...
                  try {
                    var ariaLabel = '';
                    try { ariaLabel = (typeof maskRef === 'function') ? maskRef(fullRef) : '';} catch (e) { ariaLabel = ''; }
                    if (ariaLabel) copyBtn.setAttribute('aria-label', ariaLabel);
                  } catch (e) { /* ignore */ }
                  try { copyBtn.textContent = 'Copy reference'; } catch (e) { /* ignore */ }

                  // Click handler: layered, non-throwing copy attempts
                  try {
                    copyBtn.addEventListener('click', function (ev) {
                      try {
                        ...
                        var toCopy = '';
                        try { toCopy = (btn && btn.getAttribute) ? (btn.getAttribute('data-ssp-full-ref') || '') : fullRef; } catch (e) { toCopy = fullRef; }
                        ...
                        try {
                          if (navigator && navigator.clipboard && typeof navigator.clipboard.writeText === 'function') {
                            navigator.clipboard.writeText(toCopy).then(function () { try { showSuccess(); } catch (e) {} try { if (btn) btn.disabled = false; } catch (e) {} }).catch(function () { try { showFail(); } catch (e) {} try { if (btn) btn.disabled = false; } catch (e) {} });
                            return;
                          }
                        } catch (e) { /* ignore */ }
