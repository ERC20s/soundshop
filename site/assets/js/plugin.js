...
(function (window, document) {
 ...
 // (file continues unchanged until the initPaidBannerCtas implementation)
      ...
      for (var i = 0; i < banners.length; i++) {
        ...
        try {
          ...
          try {
            ...
            // Add a small, idempotent "Copy reference" button when a canonical
            // order reference exists. Avoid adding the button twice per-banner by
            // checking data-ssp-paid-copy and set it after creating the button.
            try {
              var canonicalRef = '';
              ...
              if (canonicalRef) {
                try {
                  if (b.getAttribute('data-ssp-paid-copy') !== 'on') {
                      ...
                      try {
                        ...
                        try {
                          if (SS && typeof SS.copyText === 'function') {
                            try { SS.copyText(ref); showSuccess(); try { if (btn) btn.disabled = false; } catch (e) { /* ignore */ } return; } catch (e) { /* fallthrough */ }
                          }
                        } catch (e) { /* ignore */ }

                        // Try navigator.clipboard.writeText (promise)
                        try {
                          if (navigator && navigator.clipboard && typeof navigator.clipboard.writeText === 'function') {
                            navigator.clipboard.writeText(ref).then(function () { try { showSuccess(); } catch (e) { /* ignore */ } try { if (btn) btn.disabled = false; } catch (e) { /* ignore */ } }).catch(function () { try { showFail(); } catch (e) { /* ignore */ } try { if (btn) btn.disabled = false; } catch (e) { /* ignore */ } });
                            return;
                          }
                        } catch (e) { /* ignore */ }
                    ...
                    // Append the button into the same CTAs container
                    try { container.appendChild(copyBtn); } catch (e) { /* ignore */ }
                    try { b.setAttribute('data-ssp-paid-copy', 'on'); } catch (e) { /* ignore */ }
                  }
                } catch (e) { /* ignore per-banner copy create */ }
              }
            } catch (e) { /* ignore copy button logic */ }
