...
try {
  document.addEventListener('soundshop:verified-order', function (ev) {
    try {
      ...
      // Inline, visually-hidden description adjacent to the CTAs
      var descId = 'bought__cta-desc-' + ((detail && detail.id) ? String(detail.id) : Date.now().toString(36).slice(3));
      ...
      // Attempt to focus the primary CTA; swallow any exceptions
