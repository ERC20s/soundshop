      ...
      for (var i = 0; i < banners.length; i++) {
        ...
        try {
          ...
          try {
            ...
            // Add a "Copy reference" button when a usable reference exists on the
            // verified order object (order.id or order.ref). Guard so repeated
            // calls don't duplicate the button.
            try {
              var canonicalRef = '';
              try { canonicalRef = String(order.id || order.ref || order.reference || ''); } catch (e) { canonicalRef = ''; }
              if (canonicalRef) {
                // Don't create duplicate per-banner copy buttons
                try { if (b.getAttribute && b.getAttribute('data-ssp-paid-copy') === 'on') canonicalRef = ''; } catch (e) { /* ignore */ }
              }
              ...
              if (canonicalRef) {
                ...
                copyBtn.addEventListener('click', function (e) {
                  try {
                    ...
                    var toCopy = '';
                    try { toCopy = canonicalRef || ''; } catch (e) { toCopy = ''; }
                    ...
                    var showFailure = function () {
                      try {
                        if (SS && typeof SS.toast === 'function') {
                          try { SS.toast('Failed to copy'); } catch (e) { /* ignore */ }
                        }
                      } catch (e) { /* ignore */ }
                    };
                      ...
                      if (navigator && navigator.clipboard && typeof navigator.clipboard.writeText === 'function') {
                        try {
                          navigator.clipboard.writeText(toCopy).then(function () {
                            ...
                            try { showFailure(); } catch (e) { /* ignore */ }
                            try { if (btn) btn.disabled = false; } catch (e) { /* ignore */ }
                          });
                          return;
                    ...
                    try {
                      ...
                      try { showFailure(); } catch (err) { /* ignore */ }
                    }
                ...
                try { if (b.setAttribute) b.setAttribute('data-ssp-paid-copy', 'on'); } catch (e) { /* ignore */ }
              }
