  ...
  try {
    document.addEventListener('soundshop:verified-order', function (ev) {
      try {
        ...
        // Open in a new tab to preserve the purchase summary context and avoid
        // revealing masked delivery details in the current document.
        a1.setAttribute('target', '_blank');
        a1.setAttribute('rel', 'noopener noreferrer');

        // Add an aria-live status for screen readers announcing the new tab.
        var ariaStatus = document.createElement('span');
        ariaStatus.className = 'visually-hidden';
        ariaStatus.setAttribute('aria-live', 'polite');
        ariaStatus.textContent = 'Installers and delivery instructions are available in a new tab.';
        ...
        // Attempt to move keyboard focus to the primary CTA; swallow failures.
        try { if (typeof a1.focus === 'function') a1.focus(); } catch (e) { /* ignore */ }
